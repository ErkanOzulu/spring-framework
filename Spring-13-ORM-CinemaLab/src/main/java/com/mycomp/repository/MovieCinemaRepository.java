...
public interface MovieCinemaRepository extends JpaRepository<MovieCinema, Long> {
    ...
    List<MovieCinema> findAllByMovieNameContains(String name);
    ...
    List<MovieCinema>retrieveAllWithHigherThanSpecificDate(LocalDateTime date);
    // ------------------- Native QUERIES ------------------- //
   ...
   @Query(value = "SELECT COUNT(*) from movie_cinema where cinema_id=1?",nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM movie_cinema mc join cinema c on mc.cinema_id=c.id join location l on c.location_id=l.id " +
