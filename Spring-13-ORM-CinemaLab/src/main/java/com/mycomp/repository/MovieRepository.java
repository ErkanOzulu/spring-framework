...
import com.mycomp.enums.MovieState;
import com.mycomp.enums.MovieType;
...
public interface MovieRepository extends JpaRepository<Movie, Long> {
    ...
    List<Movie> findAllByPriceBetween(BigDecimal a, BigDecimal b);
    ...
    List<Movie> findAllByStateAndType(MovieState state, MovieType movieType);

    // ------------------- JPQL QUERIES ------------------- //

    //Write a JPQL query to list all movies between a range of prices
    @Query("Select m from Movie m where m.price between ?1 and ?2")
    List<Movie> getPriceBetween(BigDecimal startPrice, BigDecimal endPrice);
    ...
    Optional<Movie> movieByName(String name);
